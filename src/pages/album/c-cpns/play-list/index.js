...
export default memo(function Playlist({ playlistDetail }) {
  const list = playlistDetail.songs;
  ...
  return (
    <PlaylistWrapper>
      <div className="head">
        <div className="left">
          <span className="title">包含歌曲列表</span>
          <span className="count">{list && list.length}首歌</span>
        </div>
        <div className="right">
        ...
        <div className="list-main">
          {
            list && list.map((item, index) => {
              return (<div className="list-item" key={item.id}>
                ...
                <NavLink to={"/album?id" + (item.al && item.al.id)} className="w5 text-nowrap">{(item.al && item.al.name)}</NavLink>
              </div>)
            })
          }
        </div>
